...
class OfflineQueueService {
  ...
  Future<void> _processQueue() async {
    ...
    try {
      ...
      // Remove only successfully processed operations. Failed work stays on
      // device and retries when connectivity becomes usable again.
  ...
  void _scheduleNextRetry() {
    ...
    final nextAttempt = _queue
        .map((operation) {
          if (operation.lastRetry == null) return now;
          return operation.lastRetry!.add(_retryDelay(operation.retryCount));
        })
        .reduce(
          (earliest, candidate) =>
  ...
  Duration _retryDelay(int retryCount) {
    final multiplier = 1 << retryCount.clamp(0, 10);
    final delay = _retryBaseDelay * multiplier;
    ...
    try {
      switch (operation.type) {
        ...
        case OperationType.other:
          return await _processGenericFirestoreWrite(operation.data);
      }
    } catch (e) {
      return false;
    }
  ...
  /// Process a generic queued Firestore write.
  ///
  /// Expected data:
  /// {
  ...
  ///   fields: {...}
  /// }
  Future<bool> _processGenericFirestoreWrite(Map<String, dynamic> data) async {
    try {
      final collection = data['collection'] as String?;
      final fields = data['fields'];
      if (collection == null || collection.isEmpty || fields is! Map) {
        return false;
      }
      final operation = '${data['operation'] ?? 'set'}';
      final docId = data['docId'] as String?;
      final merge = data['merge'] != false;
      final collectionRef = FirebaseFirestore.instance.collection(collection);
      final safeFields = Map<String, dynamic>.from(fields);

      if (operation == 'add' || docId == null || docId.isEmpty) {
        await collectionRef.add(safeFields);
      } else if (operation == 'update') {
        await collectionRef.doc(docId).update(safeFields);
      } else {
        await collectionRef
            .doc(docId)
            .set(safeFields, SetOptions(merge: merge));
      }
      return true;
    } catch (e) {
      return false;
