...
class PatientReferralsScreen extends StatefulWidget {
  final String? userId;
  const PatientReferralsScreen({super.key, this.userId});
    ...
    ReferralService.getPatientReferrals(patientId: _currentUserId!).listen(
      (snapshot) {
        final referrals = snapshot.docs
            ...
            .where((referral) {
              // Filter out referrals that have been booked (appointment created)
              // Check if referral has appointmentId or appointmentStatus == 'booked'
              final doc = snapshot.docs.firstWhere((d) => d.id == referral.id);
              final data = doc.data() as Map<String, dynamic>;
              final appointmentStatus = data['appointmentStatus'];
              final appointmentId = data['appointmentId'];

              // Only show referrals that haven't been booked yet
              return appointmentStatus != 'booked' && appointmentId == null;
            })
            .toList();
        setState(() {
