...
class DoctorClinicalResourcesScreen extends StatefulWidget {
  ...
  @override
  State<DoctorClinicalResourcesScreen> createState() =>
      _DoctorClinicalResourcesScreenState();
}

class _DoctorClinicalResourcesScreenState
    extends State<DoctorClinicalResourcesScreen>
      ...
      if (kIsWeb) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Web download is not supported in mobile build.'),
            ),
          );
        }
      } else {
  ...
  void _playVideoInline(String url, String title) async {
    if (url.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Video URL is missing.')));
      return;
    }
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                  isDownloading ? 'Downloading...' : 'Download & Open',
                ),
                style: ElevatedButton.styleFrom(
  ...
  /// Build help videos tab for app tutorials
  Widget _buildHelpVideosTab() {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          .collection('help_videos')
          .where('targetUsers', arrayContains: 'doctor')
          .where('isActive', isEqualTo: true)
          .orderBy('category')
          .orderBy('order')
          .snapshots(),
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        _getHelpCategoryIcon(category),
                        color: _getHelpCategoryColor(category),
                      ),
                      const SizedBox(width: 8),
                      Text(
                        _getHelpCategoryDisplayName(category),
                        style: const TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                        ),
  ...
  /// Build help video card for doctors
  ...
  Future<void> _playHelpVideo(
    ...
    try {
      ...
      // Use existing video player functionality
      _playVideoInline(videoUrl, title);
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  /// Get help category display name
  String _getHelpCategoryDisplayName(String category) {
    switch (category) {
      case 'appointments':
  ...
  /// Get help category icon
  IconData _getHelpCategoryIcon(String category) {
    switch (category) {
      case 'appointments':
  ...
  /// Get help category color
  Color _getHelpCategoryColor(String category) {
    switch (category) {
      case 'appointments':
        return Colors.blue;
      case 'wallet':
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(icon: Icon(Icons.description), text: 'Clinical Materials'),
            Tab(icon: Icon(Icons.play_circle_outline), text: 'Help Videos'),
          ],
        ),
      ),
      body: TabBarView(
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final resources = snapshot.data!.docs;
              const showMax = 3;
              final showList = _showAllMaterials
                  ? resources
                  : (resources.length > showMax
                        ? resources.sublist(0, showMax)
                        : resources);
