...
enum SlideType { content, quiz, assignment, interaction }
...
class CourseProgress {
  ...
  final Map<String, String>
  interactionAnswers; // interaction slideId -> learner answer
  final Map<String, String>
  interactionFeedback; // interaction slideId -> AI feedback
  ...
  CourseProgress({
    ...
    this.interactionAnswers = const {},
    this.interactionFeedback = const {},
  ...
  factory CourseProgress.fromFirestore(DocumentSnapshot doc) {
    ...
    final interactionAnswersData =
        data['interactionAnswers'] as Map<String, dynamic>? ?? {};
    final interactionFeedbackData =
        data['interactionFeedback'] as Map<String, dynamic>? ?? {};
    return CourseProgress(
      ...
      interactionAnswers: interactionAnswersData.map(
        (key, value) => MapEntry(key, value?.toString() ?? ''),
      ),
      interactionFeedback: interactionFeedbackData.map(
        (key, value) => MapEntry(key, value?.toString() ?? ''),
      ),
  ...
  factory CourseProgress.fromMap(Map<String, dynamic> map, String id) {
    ...
    final interactionAnswersData =
        map['interactionAnswers'] as Map<String, dynamic>? ?? {};
    final interactionFeedbackData =
        map['interactionFeedback'] as Map<String, dynamic>? ?? {};
    return CourseProgress(
      id: id,
      userId: map['userId'] ?? '',
      courseId: map['courseId'] ?? '',
  ...
  CourseProgress copyWith({
    ...
    Map<String, String>? interactionAnswers,
    Map<String, String>? interactionFeedback,
    ...
    return CourseProgress(
      ...
      interactionAnswers: interactionAnswers ?? this.interactionAnswers,
