...
class _PatientRegistrationFormState extends State<_PatientRegistrationForm> {
  ...
  Future<void> _completeRegistration(String phone) async {
    if (widget.isCHW) {
      // CHW registers patient - NO AUTH ACCOUNT, NO OTP, NO DASHBOARD
      // Just create a patient record in Firestore
      try {
        final chwId = FirebaseAuth.instance.currentUser?.uid;
        if (chwId == null) {
          throw Exception('CHW must be logged in to register patients');
        }
        ...
        if (duplicateCheck.docs.isNotEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'A patient with phone number $normalizedPhone is already registered',
                ),
        ...
        // Create patient record (NO Firebase Auth account)
        await patientDocRef.set({
          ...
          'registeredBy': chwId,
          'registeredByRole': 'CHW',
          'createdAt': FieldValue.serverTimestamp(),
          'isActive': true,
          'hasAuthAccount': false, // No login dashboard
          'requiresOTP': false, // No OTP verification
        });
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Patient registered successfully! No login account created.',
            ),
      ...
      if (phoneUser != null) {
        ...
        // Check for duplicate phone numbers in users collection
        final duplicateCheck = await FirebaseFirestore.instance
            .collection('users')
            .where('phone', isEqualTo: normalizedPhone)
        ...
        if (duplicateCheck.docs.isNotEmpty &&
            duplicateCheck.docs.first.id != phoneUser.uid) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'A patient account with phone number $normalizedPhone already exists',
                ),
                backgroundColor: Colors.orange,
                duration: const Duration(seconds: 4),
              ),
            ...
            if (widget.isCHW) ...[
              TextFormField(
                ...
                decoration: const InputDecoration(
                  ...
                  border: OutlineInputBorder(),
                  hintText: 'Enter email if available',
                ),
                validator: (val) {
                  if (val == null || val.isEmpty) {
                    return null; // Optional for CHW
                  }
                      ...
                      if (widget.isCHW) {
                        final confirmed = await showDialog<bool>(
                          ...
                          builder: (context) => AlertDialog(
                            ...
                            content: Text(
                              widget.isCHW
                                  ? 'Are you sure you want to register this patient? Patient will NOT receive login credentials - they will only exist as your registered patient record.'
