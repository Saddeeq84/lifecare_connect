...
class _CHWCourseDetailsScreenState extends State<CHWCourseDetailsScreen> {
  ...
  Future<void> _loadCourseData() async {
    try {
      ...
      await _cacheCourseProgress(_courseProgress);
      ...
      _slides = slidesSnapshot.docs
          ...
          .toList();
      await _cacheCourseSlides(_slides);
      ...
      final userId = await _currentLearnerId();
      if (userId != null) {
        await _loadCachedCourseData(userId);
      }
      if (mounted) {
        final message = _slides.isEmpty
            ? 'Could not load course data: $e'
            : 'Showing saved course content. Progress will sync when online.';
  ...
  Future<bool> _enrollCourse() async {
    try {
      ...
      await _offlineQueue.queueOperation(
        ...
        data: {
          'collection': 'courseProgress',
          'docId': _courseProgress.id,
          'operation': 'set',
          'merge': true,
          'fields': _jsonSafeMap(_courseProgress.toMap()),
        },
      );
      if (!mounted) return false;
      setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Enrolled on this device. This will sync when internet improves.',
          ),
  ...
  Future<void> _cacheCourseProgress(CourseProgress progress) async {
    await _cache.set(
      key: 'course_progress_${progress.id}',
      data: _jsonSafeMap(progress.toMap()),
      expiry: const Duration(days: 30),
    );
    await _cache.set(
      key: 'course_progress_user_${progress.userId}_${progress.courseId}',
      data: _jsonSafeMap(progress.toMap()),
      expiry: const Duration(days: 30),
    );
  }

  Future<void> _cacheCourseSlides(List<CourseSlide> slides) async {
    await _cache.set(
      key: 'course_slides_${widget.course.id}',
      data: slides.map(_slideCacheMap).toList(),
      expiry: const Duration(days: 30),
    );
  }

  Future<void> _loadCachedCourseData(String userId) async {
    final progressData = await _cache.get(
      'course_progress_user_${userId}_${widget.course.id}',
    );
    if (progressData is Map) {
      _courseProgress = CourseProgress.fromMap(
    ...
    final slidesData = await _cache.get('course_slides_${widget.course.id}');
    if (slidesData is List) {
      _slides = slidesData
          .whereType<Map>()
          .map(
            (item) => CourseSlide.fromMap(
  ...
  Map<String, dynamic> _slideCacheMap(CourseSlide slide) {
    return {
      ...slide.toMap(),
      'id': slide.id,
      'createdAt': slide.createdAt.toIso8601String(),
    };
  }
