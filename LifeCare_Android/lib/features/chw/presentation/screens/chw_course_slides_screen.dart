...
class _CHWCourseSlidesScreenState extends State<CHWCourseSlidesScreen> {
  ...
  bool _offlineProgressMessageShown = false;
  String? _videoError;
  ...
  Widget _buildSlideContent() {
    ...
    if (_currentSlide.type == SlideType.interaction) {
      return _buildInteractionSlide();
    }
      ...
      child: Column(
        ...
        children: [
          ...
          if (_currentSlide.imageUrl != null &&
            ...
            _buildSlideImageGate(_currentSlide.imageUrl!),
          if (_currentSlide.imageUrl != null &&
              _currentSlide.imageUrl!.isNotEmpty)
            const SizedBox(height: 20),
          Text(
  ...
  Widget _buildSlideImageGate(String imageUrl) {
    if (!_lowDataMode.enabled || _loadedImageUrls.contains(imageUrl)) {
      return _buildSlideImage(imageUrl);
    }
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 18),
      ...
      child: Column(
        children: [
          Icon(Icons.image_outlined, color: Colors.teal.shade700, size: 36),
          const SizedBox(height: 8),
          Text(
            'Image hidden to save data. The lesson text is available offline after loading.',
            textAlign: TextAlign.center,
            style: TextStyle(color: Colors.teal.shade900, fontSize: 13),
          ),
          const SizedBox(height: 10),
          OutlinedButton.icon(
            onPressed: () => setState(() => _loadedImageUrls.add(imageUrl)),
  ...
  Widget _buildInteractionSlide() {
    final savedAnswer = _courseProgress.interactionAnswers[_currentSlide.id];
    final savedFeedback = _courseProgress.interactionFeedback[_currentSlide.id];
    final promptText = _cleanSlideContent(
      (_currentSlide.content ?? _currentSlide.description ?? '').toString(),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          if (promptText.isNotEmpty) ...[
            Text(promptText, style: const TextStyle(fontSize: 16, height: 1.6)),
            const SizedBox(height: 20),
          ],
          TextField(
          ...
          ElevatedButton.icon(
            onPressed: _isSubmitting ? null : () => _submitInteractionAnswer(),
  ...
  Future<void> _nextSlide() async {
    ...
    if (_currentSlide.type == SlideType.interaction) {
      await _submitInteractionAnswer();
      return;
    }
  ...
  Future<void> _completeCurrentSlideAndAdvance() async {
    ...
    try {
      ...
      if (!widget.isAdminPreview) {
        await _saveCourseProgress(updatedProgress);
      }
  ...
  Future<void> _markCurrentVideoWatched() async {
    ...
    try {
      await _saveCourseProgressFields(updatedProgress.id, {
        'videoWatchedSlideIds': watchedSlideIds,
...
''';
  ...
  Future<void> _submitInteractionAnswer() async {
    final answer = _interactionController.text.trim();
    if (answer.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    try {
      final feedback = await _generateInteractionFeedback(answer);
      final answers = Map<String, String>.from(
        _courseProgress.interactionAnswers,
      );
      answers[_currentSlide.id] = answer;
      final feedbackMap = Map<String, String>.from(
        _courseProgress.interactionFeedback,
      );
      feedbackMap[_currentSlide.id] = feedback;

      final updatedProgress = _courseProgress.copyWith(
        interactionAnswers: answers,
        interactionFeedback: feedbackMap,
      );
      ...
      await _showInteractionFeedback(feedback);
      await _completeCurrentSlideAndAdvance();
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Interaction submit failed: $e')));
    } finally {
      if (mounted) {
        setState(() => _isSubmitting = false);
      }
  ...
  Future<String> _generateInteractionFeedback(String answer) async {
    await GeminiService.initialize();
    final service = GeminiService.instance;
    if (!service.isConfigured) {
      throw Exception('Gemini is not configured.');
    ...
    final prompt =
        '''
You are a knowledgeable public health tutor. Provide an accurate, evidence-based answer to the learner's interaction prompt below.

Prompt:
${_currentSlide.content ?? _currentSlide.description}

Learner answer:
$answer

Instructions:
- Give the correct answer to the question in a concise but informative way.
- Base the response on established public health guidance and common evidence-based practice.
- Keep it clear, accurate, and suitable for a learner comparing their response.
- Do not mention that you are AI. Do not use markdown bullets or tables.
...
''';
  ...
  Future<void> _showInteractionFeedback(String feedback) {
    final cleanFeedback = _cleanAssignmentFeedback(feedback);
    return showDialog<void>(
      context: context,
      builder: (context) => AlertDialog(
  ...
  void _loadCurrentSlideState() {
    ...
    if (_currentSlide.type == SlideType.interaction) {
      _interactionController.text =
          _courseProgress.interactionAnswers[_currentSlide.id] ?? '';
    } else {
  ...
  Future<void> _saveCourseProgress(CourseProgress progress) async {
    await _saveCourseProgressFields(progress.id, progress.toMap());
  }

  Future<void> _saveCourseProgressFields(
    String progressId,
    Map<String, dynamic> fields,
  ) async {
    final safeFields = _jsonSafeMap(fields);
    await _cache.set(
      key: 'course_progress_$progressId',
      data: safeFields,
    ...
    try {
      ...
      if (mounted && !_offlineProgressMessageShown) {
        _offlineProgressMessageShown = true;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Progress saved on this device. It will sync when internet improves.',
            ),
            backgroundColor: Colors.orange,
          ),
        );
  ...
  Map<String, dynamic> _jsonSafeMap(Map<String, dynamic> data) {
    return Map<String, dynamic>.from(
      data.map((key, value) => MapEntry(key, _jsonSafeValue(value))),
    );
  }
...
class _CHWFinalQuizScreenState extends State<CHWFinalQuizScreen> {
  ...
  Future<void> _saveCourseProgress(CourseProgress progress) async {
    final safeFields = _jsonSafeMap(progress.toMap());
    await _cache.set(
      key: 'course_progress_${progress.id}',
      data: safeFields,
      expiry: const Duration(days: 30),
    );
    ...
    try {
      ...
      if (mounted && !_offlineProgressMessageShown) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Final quiz progress saved on this device and will sync when online.',
            ),
            backgroundColor: Colors.orange,
          ),
        );
