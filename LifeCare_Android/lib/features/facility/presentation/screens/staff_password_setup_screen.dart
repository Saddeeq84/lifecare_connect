...
class StaffPasswordSetupScreen extends StatefulWidget {
  ...
  const StaffPasswordSetupScreen({
    super.key,
...
class _StaffPasswordSetupScreenState extends State<StaffPasswordSetupScreen> {
  ...
  bool _resetTokenInvalid = false;

  Map<String, dynamic>? _staffData;
  String? _facilityCollection;
  DocumentReference<Map<String, dynamic>>? _staffDocRef;

  bool get _isPasswordReset => widget.resetToken?.isNotEmpty == true;
    ...
    try {
      ...
      for (final facilityDoc in facilitiesSnapshot.docs) {
        ...
        if (staffQuery.docs.isNotEmpty) {
          ...
          final tokenIsValid = _isValidResetToken(staffData);

          if (_isPasswordReset && !tokenIsValid) {
            setState(() {
              _resetTokenInvalid = true;
              _isLoading = false;
            });
            return;
          }

          // Check if password is already set during first-time setup.
          ...
          setState(() {
            ...
            _staffDocRef = staffDoc.reference;
            _isLoading = false;
          });
          return;
        }
  ...
  bool _isValidResetToken(Map<String, dynamic> staffData) {
    final expectedToken = staffData['passwordResetToken'] as String?;
    final expiry = staffData['passwordResetExpiry'];
    if (expectedToken == null || expectedToken != widget.resetToken) {
      return false;
    }
  ...
  Future<void> _setPassword() async {
    ...
    try {
      final staffDocRef = _staffDocRef;
      if (staffDocRef == null) {
      ...
      if (_isPasswordReset) {
        final freshStaffDoc = await staffDocRef.get();
        if (!freshStaffDoc.exists ||
            !_isValidResetToken(freshStaffDoc.data() ?? {})) {
          throw Exception('Password reset link is invalid or has expired');
        }
        ...
        await staffDocRef.update({
          ...
          'passwordResetExpiry': FieldValue.delete(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          _isPasswordReset ? 'Reset Your Password' : 'Set Up Your Password',
        ),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    const Text(
                      'This password reset link is invalid or has expired. Please request a new reset link from the login screen.',
