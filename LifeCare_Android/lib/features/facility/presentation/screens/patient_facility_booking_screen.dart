...
class _PatientFacilityBookingScreenState
  ...
  void initState() {
    super.initState();
    // If a service is pre-selected, set it up
    if (widget.preSelectedService != null) {
      _selectedServiceType =
          'facility_service_${widget.preSelectedService!['name']}';
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (widget.preSelectedService!['description'] != null &&
                          widget.preSelectedService!['description']
                              .toString()
                              .isNotEmpty) ...[
                      ...
                      Row(
                        ...
                        children: [
                          Container(
                            ...
                            child: Text(
                              widget.preSelectedService!['category'] ??
                                  'General',
                              style: TextStyle(
                                fontSize: 12,
                          ...
                          Text(
                            '₦${(widget.preSelectedService!['price'] ?? 0.0).toStringAsFixed(0)}',
                            style: TextStyle(
                              fontSize: 20,
                              fontWeight: FontWeight.bold,
                              color: Colors.green.shade700,
              ...
              // Service Type Selection (only show if no pre-selected service)
              if (widget.preSelectedService == null) ...[
                const Text(
                  'Service Type *',
  ...
  Future<void> _submitRequest() async {
    ...
    // Skip service type validation if pre-selected service exists
    if (widget.preSelectedService == null && _selectedServiceType == null) {
    ...
    try {
      ...
      String serviceType;
      String serviceLabel;
      Map<String, dynamic>? facilityServiceDetails;

      // Handle pre-selected facility service
      if (widget.preSelectedService != null) {
        final preSelected = widget.preSelectedService!;
        serviceType = 'facility_service_${preSelected['name']}';
        serviceLabel = preSelected['name'];

        // Store detailed facility service information
        facilityServiceDetails = {
          'serviceId': preSelected['id'],
          'serviceName': preSelected['name'],
          'serviceDescription': preSelected['description'],
          'serviceCategory': preSelected['category'],
          'servicePrice': preSelected['price'],
        };
      }
      // Handle manually selected service
      else if (_selectedServiceType == 'custom') {
        serviceType = 'custom';
      ...
      if (facilityServiceDetails != null &&
          facilityServiceDetails['servicePrice'] != null) {
        servicePrice = (facilityServiceDetails['servicePrice'] as num)
            .toDouble();
      }
      ...
      final requestData = {
        ...
        'shippingStatus': 'pending', // pending, processing, shipped, delivered
        'trackingNumber': null,
        'estimatedDelivery': null,
        'actualDelivery': null,
      };

      // Add facility service details if pre-selected
      if (facilityServiceDetails != null) {
        requestData['facilityService'] = facilityServiceDetails;
      }
