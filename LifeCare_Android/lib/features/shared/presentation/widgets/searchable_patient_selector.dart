...
class SearchablePatientSelector extends StatefulWidget {
  ...
  final String? currentUserRole; // 'chw', 'doctor', 'admin', 'facility'
  final String? facilityId; // For facility staff to load facility patients
  final List<String>?
  allowedPatientIds; // Optional filter for specific patient IDs
...
class _SearchablePatientSelectorState extends State<SearchablePatientSelector> {
  ...
  Future<List<Map<String, dynamic>>> _fetchPatients() async {
    // Special case for facility staff - load from facility_patients collection
    if (widget.facilityId != null) {
      try {
        final facilityPatientsSnapshot = await FirebaseFirestore.instance
        ...
        return facilityPatientsSnapshot.docs
            .map((doc) {
              final data = doc.data();
              return {
                'id': doc.id,
            ...
            .where((patient) {
              // Filter by allowed patient IDs if provided
              return widget.allowedPatientIds == null ||
                  widget.allowedPatientIds!.contains(patient['id']);
            })
            .toList();
      } catch (e) {
        return [];
    ...
    try {
      ...
      // Filter by allowed patient IDs if provided
      if (widget.allowedPatientIds != null) {
        patients = patients
            .where(
              (patient) => widget.allowedPatientIds!.contains(patient['id']),
            )
            .toList();
      }
