...
class AppointmentService {
  ...
  static Future<String> createAppointment({
    ...
    String? facilityId,
    String? facilityName,
    String? department, // Department for routing appointments
  }) async {
    try {
      // Route departments to appropriate staff approval
      String finalStatus = 'pending';
      String? routedDepartment;

      // ONLY route to OPD/Specialist if this is a FACILITY appointment
      // Remote patient to remote doctor appointments stay as 'pending'
      if (facilityId != null && department != null) {
        final deptLower = department.toLowerCase();

        // List of specialist departments that should be routed to specialist department
        final List<String> specialistDepartments = [
          'emergency',
          'pediatrics',
          'obstetrics',
          'cardiology',
        ...
        // Route General Consultation to OPD (FACILITY appointments only)
        if (deptLower.contains('general consultation')) {
          finalStatus = 'pending_opd_approval';
          routedDepartment = 'OPD';
          print(
            '🏥 Routing General Consultation appointment to OPD staff for approval',
          );
        }
        // Route specialist departments to Specialist Department (FACILITY appointments only)
        else if (specialistDepartments.any(
          (dept) => deptLower.contains(dept),
        )) {
          finalStatus = 'pending_specialist_approval';
          routedDepartment = 'SPECIALIST';
          print(
            '🏥 Routing $department appointment to Specialist Department for approval',
          );
        }
      }
      ...
      final appointmentData = {
        ...
        'notes': notes,
        'status':
            finalStatus, // pending, pending_opd_approval, approved, denied, completed, cancelled
        ...
        // Add routing info for department-specific approvals
        'routedToDepartment': ?routedDepartment,
        if (routedDepartment == 'OPD') 'requiresOpdApproval': true,
        if (routedDepartment == 'SPECIALIST')
  ...
  /// Get appointments pending OPD approval
  static Stream<QuerySnapshot> getOpdPendingAppointments(String facilityId) {
    return _firestore
        .collection('appointments')
        .where('facilityId', isEqualTo: facilityId)
        .where('status', isEqualTo: 'pending_opd_approval')
        .orderBy('createdAt', descending: true)
        .snapshots();
  }

  /// Approve appointment from OPD staff
  static Future<void> approveOpdAppointment({
    required String appointmentId,
    required String opdStaffId,
    required String opdStaffName,
    String? notes,
  }) async {
    try {
      await _firestore.collection('appointments').doc(appointmentId).update({
        'status': 'approved',
        'approvedBy': opdStaffId,
        'approvedByName': opdStaffName,
        'approvedAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
        'approvalNotes': ?notes,
      });

      // Create consultation for approved OPD appointment
      await _createConsultationFromAppointment(appointmentId);

      print('✅ OPD appointment $appointmentId approved by $opdStaffName');
    } catch (e) {
      throw Exception('Failed to approve OPD appointment: $e');
    }
  }

  /// Deny appointment from OPD staff
  static Future<void> denyOpdAppointment({
    required String appointmentId,
    required String opdStaffId,
    required String opdStaffName,
    required String reason,
    ...
    try {
      await _firestore.collection('appointments').doc(appointmentId).update({
        'status': 'denied',
        'deniedBy': opdStaffId,
        'deniedByName': opdStaffName,
        'deniedAt': FieldValue.serverTimestamp(),
        'denialReason': reason,
        'updatedAt': FieldValue.serverTimestamp(),
      });

      print('❌ OPD appointment $appointmentId denied by $opdStaffName');
    } catch (e) {
      throw Exception('Failed to deny OPD appointment: $e');
    }
  }
}
