...
class PrivacyScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
            ...
            child: Column(
              ...
              children: [
                Center(
                  child: Column(
                    children: [
                      const Text('LifeCare Connect – Privacy Policy', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.teal)),
                      const SizedBox(height: 8),
                      Text('Effective Date: August 5, 2025', style: TextStyle(fontSize: 15, color: Colors.grey[700])),
                      Text('Developed by: Rural Health Mission Nigeria', style: TextStyle(fontSize: 15, color: Colors.grey[700])),
                    ],
                  ),
                ),
                const SizedBox(height: 18),
                _sectionTitle('Introduction'),
                _sectionBody('Welcome to LifeCare Connect. We are committed to protecting your personal information and maintaining transparency in how we collect, use, and safeguard your data. This Privacy Policy explains how we handle your information in accordance with Google Play requirements, applicable health privacy regulations, and international best practices.'),
                const SizedBox(height: 18),
                _sectionTitle('Data We Collect'),
                _sectionBody('We collect only the data necessary to deliver healthcare services and support health workers:'),
                _subSectionTitle('1. Personal Information'),
                _sectionBody('Name, email address, phone number, user ID, and address.'),
                _subSectionTitle('2. Health Information (Sensitive Data)'),
                _sectionBody('Patient health conditions and medical history.\nAntenatal and postnatal visit records.\nReferrals, prescriptions, and treatment plans.\nLaboratory, radiology, physiotherapy, and other service records.\nReproductive and sexual health service details (e.g., maternal health, family planning).\nPublic health program participation (e.g., vaccination, disease prevention activities).'),
                _subSectionTitle('3. Appointment and Service Information'),
                _sectionBody('Booking details for healthcare services, including consultations, physiotherapy, laboratory, emergency care, and pharmacy services.'),
                _subSectionTitle('4. Messages and Communication Data'),
                _sectionBody('In-app chats, consultation messages, and support requests.'),
                _subSectionTitle('5. Technical and Usage Data'),
                _sectionBody('Device information, app performance logs, and feature usage statistics.'),
                _subSectionTitle('6. Location Data'),
                _sectionBody('Approximate location to assist in service delivery and referrals.'),
                const SizedBox(height: 18),
                ...
                _sectionBody('LifeCare Connect works offline. Health data is stored securely on your device and automatically encrypted before syncing to our secure servers when you reconnect to the internet.'),
                const SizedBox(height: 18),
                ...
                _sectionBody('We retain your data only for as long as necessary to provide services or comply with legal obligations. Health records may be retained for the minimum period required under applicable health regulations.'),
                const SizedBox(height: 18),
                _sectionTitle('Compliance'),
                _sectionBody('LifeCare Connect complies with the Nigeria Data Protection Regulation (NDPR), relevant health data privacy laws, and Google Play’s Health Content and Services Policy.'),
                const SizedBox(height: 18),
                _sectionTitle('Changes to This Policy'),
                _sectionBody('We may update this Privacy Policy from time to time. You will be notified within the app whenever significant changes are made.'),
                const SizedBox(height: 18),
                _sectionTitle('Contact Us'),
                _sectionBody('If you have any questions, feedback, or complaints, please contact:'),
                const Row(
                  children: [
                    Icon(Icons.email, color: Colors.teal),
                    SizedBox(width: 8),
                    Text('contat_lifecare@rhemn.org.ng', style: TextStyle(fontSize: 16, color: Colors.teal, fontWeight: FontWeight.bold)),
                  ],
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    ...
                    InkWell(
                      child: const Text('https://lifecare-connect.web.app', style: TextStyle(fontSize: 16, color: Colors.teal, decoration: TextDecoration.underline)),
                      onTap: () {
    ...
    Widget _sectionTitle(String title) {
      return Text(title, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.teal));
    }
      ...
      return Column(
        ...
        children: items.map((item) => Padding(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('• ', style: TextStyle(fontSize: 16, color: Colors.teal)),
