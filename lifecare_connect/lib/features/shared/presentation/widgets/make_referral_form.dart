...
class _MakeReferralFormState extends State<MakeReferralForm> {
  ...
  Future<void> _submitReferral() async {
    ...
    try {
      ...
      debugPrint('[Referral Debug] Submitting referral with values:');
      debugPrint('  patient_id: [33m$_selectedPatientId[0m, patient_name: $_selectedPatientName');
      debugPrint('  referring_provider: ${user.uid}, role: ${widget.role}');
      debugPrint('  doctor_id: $_selectedDoctorId, doctor_name: $_selectedDoctorName');
      debugPrint('  specialty: $_selectedSpecialty, urgency: $_selectedUrgency, date: $_selectedDate');
      ...
      debugPrint('[Referral Debug] Error submitting referral: $e');
      debugPrint('[Referral Debug] Stack trace: $stack');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  return Column(
                    ...
                    children: [
                      const Text('Error loading doctors. Please try again.', style: TextStyle(color: Colors.red)),
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return Column(
                    ...
                    children: [
                      ...
                      Text('Debug: users count = \\${snapshot.data?.docs.length ?? 0}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
                    ],
                  );
                }
                final users = snapshot.data!.docs;
                // For debugging: show all users as selectable doctors
                if (users.isEmpty) {
