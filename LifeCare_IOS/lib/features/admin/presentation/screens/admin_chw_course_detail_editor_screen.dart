...
class _AdminCHWCourseDetailEditorScreenState
  ...
  static const List<String> roles = ['chw', 'doctor', 'patient'];
  ...
  void _loadCourseData() async {
    ...
    try {
      ...
      if (courseDoc.exists) {
        ...
        _selectedCategory = data['category'] ?? 'maternal_health';
...
class _SlideEditorDialogState extends State<SlideEditorDialog> {
  ...
  static const List<String> slideTypes = ['content', 'quiz', 'video', 'image'];
  ...
  Future<void> _pickMedia() async {
    final result = await FilePicker.platform.pickFiles(
      type: _selectedType == 'video' ? FileType.video : FileType.image,
      allowMultiple: false,
    );
