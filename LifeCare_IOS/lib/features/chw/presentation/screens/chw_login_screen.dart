...
class _CHWLoginScreenState extends State<CHWLoginScreen> {
  ...
  final AutoRepairMissingUsersService _autoRepair =
      AutoRepairMissingUsersService();
  ...
  Future<void> handleLogin() async {
    ...
    try {
      ...
      // This prevents conflicts when the same email is used for both CHW and facility staff
      ...
      final user = await _authService.signInWithEmail(
        ...
        skipEmailVerification: true, // CHWs don't need email verification
      );
      ...
      if (!doc.exists || doc.data() == null) {
        ...
        _showSnackBar('Account data missing. Attempting automatic repair...');
        ...
        if (repaired) {
          _showSnackBar('Account repaired successfully! Please log in again.');
          await _authService.signOut();
          setState(() => isLoading = false);
          return;
        } else {
          _showSnackBar(
            'No user profile found and repair failed. Contact admin.',
          );
          setState(() => isLoading = false);
          return;
        }
