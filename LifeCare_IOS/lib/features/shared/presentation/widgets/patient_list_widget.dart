...
class _PatientListWidgetState extends State<PatientListWidget> {
  ...
  Stream<QuerySnapshot> _buildInteractedPatientsQuery(String userId) async* {
    try {
      ...
      // 1. Patients from health records (consultations, ANC visits, etc.)
      ...
      for (final doc in healthRecords.docs) {
        ...
        if (data['patientUid'] != null &&
            data['patientUid'].toString().isNotEmpty) {
          patientIds.add(data['patientUid']);
        }
      }

      // 2. Patients from completed appointments
      try {
      ...
      // 3. Patients from referrals (if user referred or received patients)
      try {
      ...
      // Filter out empty or null IDs to prevent Firestore errors
      final List<String> patientIdsList = patientIds
          .where((id) => id.isNotEmpty)
          .toList();

      // If all IDs were invalid, return empty result
