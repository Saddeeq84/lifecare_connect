...
exports.walletPayment = functions.https.onCall(async (data, context) => {
  ...
  // Fetch dynamic revenue share percentages from Firestore
  let providerSharePercent = 0.7; // Default fallback
  let adminSharePercent = 0.3;    // Default fallback
  
  try {
    const feeConfigDoc = await admin.firestore()
    ...
    if (feeConfigDoc.exists) {
      ...
      if (feeConfig.revenueShare) {
        providerSharePercent = (feeConfig.revenueShare.providerPercent || 70) / 100;
        adminSharePercent = (feeConfig.revenueShare.adminPercent || 30) / 100;
      }
    }
  } catch (error) {
    console.error('Error fetching fee config, using defaults:', error);
  }
  
  const providerShare = Math.round(totalFee * providerSharePercent);
